...
const push = (n, a) => { closeSheet(); ui.stacks[ui.tab].push({ n, a: a || {} }); render(0, 'fwd'); };
function applyA11y() {
...
function openSheet(html) { const s = $('#sheet'); s.innerHTML = `<div class="scrim" data-act="closesheet"></div><div class="sheetc" role="dialog" aria-modal="true">${html}</div>`; s.hidden = false; const b = s.querySelector('button,select,input'); if (b) b.focus({ preventScroll: true }); }
...
/* ---------- actions ---------- */
const A = {
  start() { ui.started = true; render(0, 'fwd'); },
  golog() { ui.pre = 'login'; render(0, 'fwd'); },
  gosign() { ui.pre = 'signup'; render(0, 'fwd'); },
  gosplash() { ui.pre = 'splash'; render(0, 'back'); },
  dologin() { if (!S.onboarded) { ui.onbStep = 0; ui.pre = 'onboard'; render(0, 'fwd'); } else A.start(); },
  dosignup() { ui.onbStep = 0; ui.pre = 'onboard'; render(0, 'fwd'); },
  gmail() {
    toast('Signed in with Gmail (prototype)');
    if (!S.onboarded) { ui.onbStep = 0; ui.pre = 'onboard'; render(0, 'fwd'); } else A.start();
  },
  onbnext() { ui.onbStep = (ui.onbStep || 0) + 1; render(0, 'fwd'); },
  ...
  back() { const s = ui.stacks[ui.tab]; if (s.length > 1) { s.pop(); render(0, 'back'); } },
  tab(d) { if (d.t === ui.tab) ui.stacks[d.t].length = 1; ui.tab = d.t; closeSheet(); render(0); },
...
<button class="btn2 full" data-act="usedefaddr">Use default: 14 Dwarka Greens, Delhi 110078</button>`);
  ...
  usedefaddr() { S.addr = { label: 'Home', line: '14 Dwarka Greens', city: 'Delhi', pin: '110078' }; save(); closeSheet(); render(1); toast('Delivery location set to Delhi 110078'); },
  ...
  bought(d) {
    ...
    closeSheet(); ui.tab = 'profile'; ui.stacks.profile = [{ n: 'profile', a: {} }, { n: 'hist', a: {} }]; render(0, 'fwd'); toast('Added to Purchase History');
  },
  ...
  mkalert() { if (setAlert($('#nap').value, Math.round(+$('#nat').value))) { closeSheet(); render(1); toast('Alert created'); } },
  sim(d) {
  ...
  logout() { ui.started = false; ui.tab = 'home'; ui.pre = 'splash'; Object.keys(ui.stacks).forEach(k => ui.stacks[k].length = 1); render(0); },
...
document.addEventListener('submit', e => {
  ...
  if (f === 'sform') doSearch($('#q').value);
    ...
    save(); closeSheet(); render(1); toast('Delivery location updated');
  }
});
