...
V.home = () => {
  const un = unread(), ad = S.addr, dark = effTheme() === 'dark';
...
<button class="ib on-strong" data-act="theme" aria-label="${dark ? 'Switch to light theme' : 'Switch to dark theme'}">${ic(dark ? 'sun' : 'moon', 20)}</button>
...
V.profile = () => {
  ...
  return `<header class="hg prof mesh-cool"><div class="pav">${ic('user', 26)}</div><div><h1>${esc(S.account.name)}</h1><p>${esc(S.account.email)}</p></div></header><div class="scroll pad gap">
...
V.notifs = () => `${hdrP('Notifications', S.notifs.length ? 'Price drops, fast-delivery picks and recommendations' : '', S.notifs.some(n => !n.read) ? '<button class="lnk" data-act="readall">Mark all read</button>' : '')}<div class="scroll pad">
${S.notifs.length ? S.notifs.map(n => `<button class="ncard ${n.read ? '' : 'new'}" data-act="nopen" data-id="${n.id}"><span class="ni ${n.type}">${ic(n.type === 'drop' ? 'tdown' : n.type === 'fast' ? 'box' : 'spark', 16)}</span><div class="grow"><b>${n.title}</b><span>${esc(n.body)}</span><small class="mute">${ago(n.ts)}</small></div>${n.read ? '' : '<i class="dotn" aria-label="Unread"></i>'}</button>`).join('') : empty('bell', 'Nothing yet', 'Price drops and delivery updates for your wishlist will show up here.')}</div>`;

V.settings = () => `${hdrG('Settings', 'Manage your account, notifications and accessibility')}<div class="scroll pad gap"><h3 class="cap">Preferences</h3><div class="bento2">${gtile('bell', 'blue', 'Notifications', 'notifs', '', 'Drops & recs', unread())}${gtile('eye', 'violet', 'Accessibility', 'access', '', 'Theme, colour & text')}</div>
...
V.access = () => {
  ...
  const themes = [['system', 'Match System', 'Follow your device setting'], ['light', 'Light', 'Always use the light theme'], ['dark', 'Dark', 'Always use the dark theme']];
  return `${hdrP('Accessibility Settings', 'Customize your viewing experience')}<div class="scroll pad gap">
<section class="fc"><div class="ah2"><span class="mi">${ic(effTheme() === 'dark' ? 'moon' : 'sun', 18)}</span><div><h2>Appearance</h2><span class="mute">Light or dark theme</span></div></div><div class="opts" role="radiogroup" aria-label="Appearance">${themes.map(t => `<button class="opt two ${S.theme === t[0] ? 'on' : ''}" data-act="setTheme" data-v="${t[0]}" role="radio" aria-checked="${S.theme === t[0]}"><span class="grow"><b>${t[1]}</b><small>${t[2]}</small></span><span class="rd" aria-hidden="true"></span></button>`).join('')}</div></section>
...
V.ai = () => `${hdrG('AI Shopping Assistant', 'Answers from the sample catalogue', '', 'ai', false)}<div class="scroll chat" id="chat" aria-live="polite">
